...
def get_agent_map_data(env):
    event = env.step({"action": "ToggleMapView", "agentId": 0})
...
def add_agent_view_triangle(
    ...
    offset1 = scale * np.array([-1, 0.5]) * math.sqrt(2) / 2 # + - ? control the width of the triangle
    offset2 = scale * np.array([1, 0.5]) * math.sqrt(2) / 2
    ...
    draw.polygon(points, fill=(0, 255, 0, opacity))
...
def visualize_agent_path(
    ...
    if colors is None:
        ...
        # colors = [col.Color("green")] * len(positions)
...
def draw_rectangle_on_image(img, center, width, color=(0, 255, 0)):
